...
/**
 * Create a recursive descent parser that adheres to RPAL's phrase structure grammar.
 * Receive input from the scanner for each clause in the phrase structure.
 * grammar.
...
public class Parser {
  ...
  private void readNT() {
    do {
      currentToken = s.readNextToken(); // Get the next token
  ...
  private void procBP() {
    procA(); // Bp -> A
    if (isCurrentToken(TokenType.RESERVED, "gr") || isCurrentToken(TokenType.OPERATOR, ">")) { // Bp -> A('gr' | '>' ) A
                                                                                               // => 'gr'
      readNT();
      procA(); // extra readNT in procA()
      buildNAryASTNode(ASTNodeType.GR, 2);
    } else if (isCurrentToken(TokenType.RESERVED, "ge") || isCurrentToken(TokenType.OPERATOR, ">=")) { // Bp -> A ('ge'
                                                                                                       // | '>=') A =>
                                                                                                       // 'ge'
      readNT();
      procA(); // extra readNT in procA()
      buildNAryASTNode(ASTNodeType.GE, 2);
    } else if (isCurrentToken(TokenType.RESERVED, "ls") || isCurrentToken(TokenType.OPERATOR, "<")) { // Bp -> A ('ls' |
                                                                                                      // '<' ) A => 'ls'
      readNT();
      procA(); // extra readNT in procA()
      buildNAryASTNode(ASTNodeType.LS, 2);
    } else if (isCurrentToken(TokenType.RESERVED, "le") || isCurrentToken(TokenType.OPERATOR, "<=")) { // Bp -> A ('le'
                                                                                                       // | '<=') A =>
                                                                                                       // 'le'
      readNT();
      procA(); // extra readNT in procA()
  ...
  private void procDB() {
    if (isCurrentTokenType(TokenType.L_PAREN)) { // Db -> '(' D ')'
      ...
      if (isCurrentToken(TokenType.OPERATOR, ",")) { // Db -> Vl '=' E => '='
        ...
        if (isCurrentToken(TokenType.OPERATOR, "=")) { // Db -> Vl '=' E => '='; if Vl had only one IDENTIFIER (no
                                                       // commas)
          readNT();
          procE(); // extra readNT in procE()
          buildNAryASTNode(ASTNodeType.EQUAL, 2);
